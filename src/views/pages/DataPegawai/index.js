...
const fields = ['no','photo','nip','nama','jenis_kelamin','tempat_lahir','tgl_lahir','no_telp','email','alamat', 'show_details'];
...
const DataPegawai = props => {
    ...
    React.useEffect(()=> {
      dispatch(getAllPegawai()).then(res => {
        console.log(res);
      }).catch(err => {
        console.log(err);
      })
                ...
                'show_details':
                  ...
                  'details':
                    (item, index)=>{
                        return (
                            ...
                            <CCollapse show={details.includes(index)}>
                                ...
                                <Link className="btn btn-success "  to={`data-pegawai/${item.id}`} >
                                    Details
                                </Link>
                                <Link className="btn btn-warning ml-1"  to={`data-pegawai/update/${item.id}`} >
                                    Update
                                </Link>
                                <CButton size="md" color="danger" className="ml-1" onClick={()=> deleteData(item.id)}  >
                                    Delete
                                </CButton>
                                </CCardBody>
                            </CCollapse>
                    ...
                    (item) => (
                      <td>
                        <CImg
                          src={'http://localhost:8080/public/photo_file/' + item.photo_file}
                          className="c-avatar-img"
                          alt="admin@bootstrapmaster.com"
                          style={{width: 30, height: 30, backgroundPosition: 'center', backgroundSize: 'cover'}}
                        />
                      </td>
                    )
